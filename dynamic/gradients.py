...
def gradient_CCL(dif,sv, Jl_ccl_cgdl, Hcl, Sl_ccl, Jv_ccl_cgdl, S_sorp_ccl, Sv_ccl, J_O2_ccl_cgdl, J_O2_mem_ccl, Sp_ccl,
    ...
    M_Pt0 = 4 / 3 * np.pi * rho_Pt * np.trapz(y=prd0 * r_m ** 3, x=r_m)
    dMdisdt = 4 * np.pi * rho_Pt * np.trapz(y=prd_ccl * r_m ** 2 * drdt, x=r_m)
    dMcdisdt = 4 * np.pi * rho_Pt * np.trapz(y=prd_ccl * r_m ** 2 * kcdis, x=r_m)
