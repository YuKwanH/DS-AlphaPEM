...
nodes_names_vp = [f"C_v_agc"] + [f"C_v_agdl_{i+1}" for i in range(parameters["n_gdl"])] + ["C_v_acl"] + ["C_v_ccl"] + [f"C_v_cgdl_{i+1}" for i in range(parameters["n_gdl"])] + ["C_v_cgc"]
nodes_name_O2 = [f"C_O2_agc"] + [f"C_O2_mem_{i+1}" for i in range(parameters["n_mem"])] + ["C_O2_ccl"] + [f"C_O2_cgdl_{i+1}" for i in range(parameters["n_gdl"])] + ["C_O2_cgc"]
nodes_names_H2 = [f"C_H2_agc"] + [f"C_H2_agdl_{i+1}" for i in range(parameters["n_gdl"])] + ["C_H2_acl"] + [f"C_H2_mem_{i+1}" for i in range(parameters["n_mem"])] + ["C_H2_ccl"] + [f"C_H2_cgdl_{i+1}" for i in range(parameters["n_gdl"])] + ["C_H2_cgc"]
