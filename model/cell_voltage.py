...
def calculate_cell_voltage_intermediate(ECSA, theta, variables, operating_inputs, parameters):
    ...
    lambda_mem, lambda_ccl = variables["lambda_mem"], variables["lambda_ccl"]
    C_H2_acl, C_O2_ccl = variables["C_H2_acl"], variables["C_O2_ccl"]

    # The equilibrium potential
    Ueq = (E0 - 8.5e-4 * (Tfc - 298.15) + R * Tfc / (2 * F) *
           (np.log(R * Tfc * C_H2_acl / Pref) + 0.5 * np.log(R * Tfc * C_O2_ccl / Pref)))

    1  # its value is around [4-7]e-6 ohm.m².

    # The cell voltage
    return Ueq, (Rp + Re)

def calculate_cell_voltage(ECSA, theta, variables, operating_inputs, parameters):
    ...
    for i in range(n):
        ...
        # The cell voltage
        Ucell_t[i] = Ueq - (i_fc + i_n) * (Rp + Re) - eta_c /(ECSA)
    return Ucell_t
